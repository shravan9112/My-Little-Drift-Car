...
namespace Physics.Wheel
{
	...
	[CustomEditor(typeof(CarWheelCollider)), CanEditMultipleObjects]
	public class WheelColliderEditor : Editor
	{
		public override void OnInspectorGUI()
		{
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				{
					(targets[i] as CarWheelCollider).UpdateConfig();
				}
			}
		}
		...
		void OnEnable()
		{
			for (int i = 0; i < targets.Length; i++)
			{
				if ((targets[i] as CarWheelCollider).CheckFirstEnable())
				{
		...
		public class FullPropertyWheelColliderConfigDrawer : WheelColliderConfigDrawer
		{
			protected override bool IsFullProperty { get { return true; } }
		}

		[CustomPropertyDrawer(typeof(WheelColliderConfig))]
		public class WheelColliderConfigDrawer : PropertyDrawer
		{
